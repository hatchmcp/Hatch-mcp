...
// Unknown namespaces are replaced with an empty string (logged upstream).
export function substituteTemplate(
  ...
  return template.replace(/\$\{([^}]+)\}/g, (match, key: string) => {
    ...
    if (dotIdx === -1) return match // malformed — leave as-is

    const ns = key.slice(0, dotIdx)
    const field = key.slice(dotIdx + 1)

    switch (ns) {
      case 'env':   return vars.env[field]   ?? ''
      case 'input': return String(vars.input[field] ?? '')
      case 'auth':  return vars.auth[field]  ?? ''
      default:      return ''
    }
  })
}

// Deep-substitutes all string values in an object (body_template)
